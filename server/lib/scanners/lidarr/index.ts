  ...
  implements RunnableScanner<SyncStatus>
{
  ...
  public async run(): Promise<void> {
    ...
    try {
      this.servers = uniqWith(settings.lidarr, (a, b) => {
        return (
  ...
  private async processLidarrAlbum(album: LidarrAlbum): Promise<void> {
    if (
      !album.monitored &&
      !(album.statistics?.trackFileCount && album.statistics.trackFileCount > 0)
    ) {
      this.log(
        'Album is unmonitored and has no tracks downloaded. Skipping.',
        'debug',
        { title: album.title }
      );
      return;
    ...
    try {
      await this.processMusic(album.foreignAlbumId, {
        serviceId: this.currentServer.id,
        externalServiceId: album.id,
        externalServiceSlug: album.foreignAlbumId,
        title: album.title,
        processing: !(
